...
class AddFormData extends React.Component {
    ...
    numeratorChange(e) { this.setState({ numerator: e.target.value }) }
    denominatorChange(e) { this.setState({ denominator: e.target.value }) }
    ...
    render() {
        return (
            <form onSubmit={this.onFormSubmit} className='well'>
                <h1 style={{fontSize: '30px'}}>{this.props.upd.id ? 'Cập Nhật Lại Câu Hỏi' : 'Thêm Câu Hỏi'}</h1>
            ...
            <tr id={'tr-' + this.props.TRs.id.toString()} >
                ...
                <td>
                    {this.state.isToggleOn ? (
                        <button disabled className="btn btn-xs btn-default" >Chỉnh sửa</button>
                    ) : (
                    ...
                    <div className='row h35'>
                        <div className='col-md-6'>
                            <button onClick={this.delNrow} id='del_rowBtn'
                                className='btn btn-xs btn-default'
                                style={display} >Xóa câu hỏi</button>
                        </div>
